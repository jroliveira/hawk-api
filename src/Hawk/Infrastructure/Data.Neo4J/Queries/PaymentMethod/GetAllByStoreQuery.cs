namespace Hawk.Infrastructure.Data.Neo4J.Queries.PaymentMethod
{
    ...
    {
        private static readonly Option<string> Statement = CypherScript.ReadAll("PaymentMethod.GetAllByStore.cql");
        ...
        public async Task<Try<Paged<Try<(Method Method, uint Count)>>>> GetResult(string email, string store, Filter filter)
        {
            var parameters = new
            {
                email,
