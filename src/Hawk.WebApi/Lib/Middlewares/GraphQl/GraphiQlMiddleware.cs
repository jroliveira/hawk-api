namespace Hawk.WebApi.Lib.Middlewares.GraphQl
{
    ...
    public sealed class GraphiQlMiddleware
    {
        ...
        public async Task Invoke(HttpContext context)
        {
            ...
            if (this.IsGraphiQlRequest(context.Request) && this.config.Enabled)
            {
...
";
        ...
        {
            return string.Equals(request.Method, "GET", StringComparison.OrdinalIgnoreCase) && request.Path.StartsWithSegments(this.config.ManagerPath);
        }
        ...
        private Task WriteResponseAsync(HttpResponse response)
        {
            ...
            builder.Replace("@{GraphQLPath}", this.config.Path);
